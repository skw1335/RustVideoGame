...
impl<'a> System<'a> for ItemCollectionSystem {
    ...
    fn run(&mut self, data : Self::SystemData) {
        ...
        for pickup in wants_pickup.join() {
            ...
            backpack.insert(pickup.item, InBackpack { owner: pickup.collected_by }).expect("Unable to pickup");

            if pickup.collected_by == *player_entity {
                gamelog.entries.push(format!("You pickup the {}.", names.get(pickup.item).unwrap().name));
                }
            }
...
impl<'a> System<'a> for ItemUseSystem {
    ...
    fn run(&mut self, data : Self::SystemData) {
        ...
        for (entity, useitem) in (&entities, &wants_use).join() {
            ...
            // Damage Items, If it damages, apply the damage on the target cell
